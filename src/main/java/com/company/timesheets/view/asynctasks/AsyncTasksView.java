...
public class AsyncTasksView extends StandardView {
    ...
    @Subscribe(id = "performWithResultBtn", subject = "clickListener")
    public void onPerformWithResultBtnClick(final ClickEvent<JmixButton> event) {
        String typed = inputField.getValue();

        uiAsyncTasks.supplierConfigurer(() -> changeMyString(typed))
                .withResultHandler(resultString ->
                        notifications.show(resultString)
                ).withTimeout(3, TimeUnit.SECONDS)
                .withExceptionHandler(ex -> {
                    if (ex instanceof TimeoutException) {
                        notifications.create("Timeout!")
    ...
    private String changeMyString(String typed) {
        try {
            sleep(4000);
        } catch (InterruptedException e) {
            throw new RuntimeException("Interrupted", e);
